...
class Listt(QDialog):
    ...
    def generate_groupbox(self,i):
        ...
        self.pushbutton_edit.setGeometry(QtCore.QRect(470, 10, 71, 21))
        font = QtGui.QFont()
        font.setFamily("Baskerville Old Face")
        font.setPointSize(13)
        font.setBold(False)
        ...
        "border-width : 0.5px;\n"
        "border-style:inset;")
        self.pushbutton_edit.setObjectName("pushbotton_edit" + str(i))
    ...
    def accept(self):
        ...
        if len(text) == 0:
            text = product_X[userid][which_one].allproductid[which_id]
        product_X[userid][which_one].allproductid[which_id] = text
        edit_button.setVisble = (True)
        login = Listt()
        widget.addWidget(login)
        widget.setCurrentIndex(widget.currentIndex()+1)
